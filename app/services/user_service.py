...
def service_get_user(user_id: int):
    ...
    # 중복 이메일 확인
    if get_member_by_email(user.email):
        raise HTTPException(400, "이미 등록된 이메일입니다.")

    hashed_pw = bcrypt.hashpw(user.password.encode(), bcrypt.gensalt()).decode()
    now = datetime.now()

    member_id = create_member(user.name, user.email, hashed_pw, now)
    ...
    current = get_member_by_id(user_id)
    if not current:
    ...
    if user_update.email and user_update.email != current["email"]:
        if get_member_by_email(user_update.email):
            raise HTTPException(400, "이미 등록된 이메일입니다.")
    ...
    update_member(user_id, new_name, new_email, hashed_pw)
